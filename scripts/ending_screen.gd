class_name EndingScreen
extends Menu

## Shown after the last room. Totals only — the game is short enough that the
## interesting number is how fast you did all of it.

var total_time := 0.0
var total_gems := 0
var max_gems := 0
var deaths := 0

var _cube: Texture2D


func _ready() -> void:
	super()
	_cube = PixelArt.cube(8)
	title = Lang.t("ending.title")
	subtitle = Lang.t("ending.subtitle")
	list_top = 206.0
	footer = ""
	items = [
		{"id": "levels", "label": Lang.t("ending.rooms")},
		{"id": "title", "label": Lang.t("ending.menu")},
	]


func draw_header() -> void:
	var cx := SCREEN.x * 0.5
	var hover := roundf(sin(_time * 1.6) * 2.0)
	draw_texture(_cube, Vector2(roundf(cx - _cube.get_width() * 0.5), 76.0 + hover))

	var panel := Rect2(cx - 120.0, 118.0, 240.0, 74.0)
	Util.draw_panel(self, panel, Palette.BG_SOFT, Palette.FRAME)

	_row(panel, 0, Lang.t("ending.total_time"), Util.format_time(total_time), Palette.GOLD)
	_row(panel, 1, Lang.t("ending.gems"), "%d / %d" % [total_gems, max_gems],
		Palette.GOLD if total_gems >= max_gems and max_gems > 0 else Palette.GREY)
	_row(panel, 2, Lang.t("ending.deaths"), str(deaths), Palette.GREY)


func _row(panel: Rect2, i: int, label: String, value: String, color: Color) -> void:
	var y := panel.position.y + 12.0 + i * 20.0
	PixelFont.draw_text(self, label, Vector2(panel.position.x + 14.0, y), Palette.GREY_DARK, 1)
	var size := PixelFont.measure(value, 2)
	PixelFont.draw_text(self, value,
		Vector2(panel.position.x + panel.size.x - 14.0 - size.x, y - 4.0), color, 2)
